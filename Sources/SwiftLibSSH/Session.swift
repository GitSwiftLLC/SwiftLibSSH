...
public extension SwiftLibSSH {
    ...
    /// - Returns: 返回一个SSH实例。
    static func getSSH(from abstract: UnsafeRawPointer) -> SwiftLibSSH {
        return abstract.bindMemory(to: SwiftLibSSH.self, capacity: 1).pointee
    }

    // 从原始会话指针中获取SSH会话对象
    // - 参数: rawSession - 原始的libssh2会话指针
    // - 返回: 如果成功获取到SSH会话对象，则返回SSH类型，否则返回nil
    static func getSSH(from rawSession: OpaquePointer?) -> SwiftLibSSH? {
        guard let abstract = libssh2_session_abstract(rawSession) else {
            return nil
        }
        return SwiftLibSSH.getSSH(from: abstract)
    }
    ...
    func handshake() async -> Bool {
        ...
        let disconnect: disconnectType = { sess, reason, message, messageLen, language, languageLen, abstract in
            SwiftLibSSH.getSSH(from: abstract).disconnect(sess: sess, reason: reason, message: message, messageLen: messageLen, language: language, languageLen: languageLen)
        }
        let send: sendType = { socket, buffer, length, flags, abstract in
            SwiftLibSSH.getSSH(from: abstract).send(socket: socket, buffer: buffer, length: length, flags: flags)
        }
        let recv: recvType = { socket, buffer, length, flags, abstract in
            SwiftLibSSH.getSSH(from: abstract).recv(socket: socket, buffer: buffer, length: length, flags: flags)
        }
        #if DEBUG
            let debug: debugType = { sess, reason, message, messageLen, language, languageLen, abstract in
                SwiftLibSSH.getSSH(from: abstract).debug(sess: sess, reason: reason, message: message, messageLen: messageLen, language: language, languageLen: languageLen)
            }
        #endif
        let trace: libssh2_trace_handler_func = { sess, _, message, messageLen in
            ...
            SwiftLibSSH.getSSH(from: sess)?.trace(message: message, messageLen: messageLen)
        }
            ...
            return await call {
                let code = self.callSSH2 {
                    libssh2_userauth_keyboard_interactive_ex(rawSession, self.user, self.user.countUInt32) { _, _, _, _, numPrompts, prompts, responses, abstract in
                        ...
                        let ssh = SwiftLibSSH.getSSH(from: from)
