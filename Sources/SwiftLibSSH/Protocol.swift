...
public protocol ChannelDelegate {
    ...
    ///   - ssh: SwiftLibSSH实例
    ///   - data: 到达的标准输出数据
    func stdout(ssh: SwiftLibSSH, data: Data) async

    /// 标准错误输出数据到达时调用
    /// - Parameters:
    ///   - ssh: SwiftLibSSH实例
    ///   - data: 到达的标准错误输出数据
    func dtderr(ssh: SwiftLibSSH, data: Data) async

    /// 通道断开时调用
    /// - Parameter ssh: SwiftLibSSH实例
    func disconnect(ssh: SwiftLibSSH)
    ...
    ///   - ssh: SwiftLibSSH实例
    ///   - online: 通道是否在线
    func connect(ssh: SwiftLibSSH, online: Bool)
}
